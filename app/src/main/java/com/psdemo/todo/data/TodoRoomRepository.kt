...
class TodoRoomRepository(private val todoDao: TodoDao) : TodoRepository {
    ...
    override fun getUpcomingTodosCount(): LiveData<Int> = todoDao.getDateCount(System.currentTimeMillis())
    ...
    private class InsertAsyncTask(val todoDao: TodoDao) : AsyncTask<Todo, Unit, Unit>() {
        @Deprecated("Deprecated in Java", ReplaceWith("todoDao.insert(todos[0]!!)"))
    ...
    private class ToggleAsyncTask(val todoDao: TodoDao) : AsyncTask<String, Unit, Unit>() {
        @Deprecated("Deprecated in Java", ReplaceWith("require(todoDao.toggleTodo(ids[0]!!) == 1) { \"Todo not found\" }"))
