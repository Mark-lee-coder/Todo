...
class AddViewModelTest {
    @get:Rule
    private val rule = MockitoJUnit.collector()
...
//        )
        verify(repository).insert(argThat { title == actualTitle && dueDate == null })
    }
    ...
    fun test_saveWithDate() {
        ...
        assertNull(actual)
        verify(repository).insert(any())
        verify(repository).insert(argThat { title == actualTitle && dueDate == actualDate })
    }
